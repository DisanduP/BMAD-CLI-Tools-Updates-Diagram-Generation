...
function convertToDrawio(parsedDiagram, options = {}) {
  ...
  // Add Start and End nodes for flowcharts
  let nodes = [...parsedDiagram.nodes];
  let edges = [...parsedDiagram.edges];
  
  // Build adjacency maps to find roots and leaves
  ...
  const rootNodes = nodes.filter((node) => inEdges.get(node.id).length === 0);
  
  // Find leaf nodes (no outgoing edges)
  const leafNodes = nodes.filter((node) => outEdges.get(node.id).length === 0);
  ...
  rootNodes.forEach((root) => {
    edges.push({
      ...
      target: root.id,
      label: '',
      arrowType: { type: 'solid', arrow: 'classic' }
    });
  });
